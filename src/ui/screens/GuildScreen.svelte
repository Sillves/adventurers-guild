<script lang="ts">
  ...
  import { autoClickPerSecond, clickGain, comboCap, fameGain, fameMilestoneBase, fameTargetGold, incomePerSecond } from '../../engine/formulas';
  ...
  // drempel op max(totalFameFor lifetime, balans+uitgegeven): ligt altijd boven
  // huidig goud, voor zowel onclaimed Fame (normaal) als gebankte veteranen
  const milestoneBase = $derived(fameMilestoneBase(game.state));
  const nextTarget = $derived(fameTargetGold(milestoneBase + 1));
  const prevTarget = $derived(fameTargetGold(milestoneBase));
